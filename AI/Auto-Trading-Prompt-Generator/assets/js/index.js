...
const strategies = [
  ...
  },
  {
    id: "snr_smc",
    name: "SNR + SMC / ICT",
    description: "支撐阻力結合聰明錢。重點檢查 HTF SNR、流動性掃蕩、BOS/CHoCH、OB/FVG 與下一個流動性 TP。"
  },
  {
    id: "reversal",
    name: "反轉交易",
...
function buildStrategyFrameworkSection() {
  if (state.strategy === "snr_smc") {
    return `## Selected strategy skill: snr-smc-strategy

When evaluating the setup, use the local skill \`skills/snr-smc-strategy/SKILL.md\`.

SNR + SMC rules:

1. Use HTF first: mark trend, major SNR, fresh/unfresh SNR, RBS/SBR, previous highs/lows, equal highs/lows, and session/day highs/lows when visible.
2. Identify buy-side liquidity above highs/resistance/equal highs and sell-side liquidity below lows/support/equal lows.
3. Do not enter directly at SNR, OB, or FVG. Wait for sweep/rejection, breakout retest, BOS, CHoCH, MSS, or valid retest confirmation.
4. Select exactly one setup model: SNR_BOUNCE, BREAKOUT_RETEST, LIQUIDITY_SWEEP, OB_FVG_CONFIRMATION, RANGE_ROTATION, TREND_PULLBACK, or NONE.
5. Stop must be structural: beyond sweep high/low, OB/FVG edge, retest base, or invalidation level.
6. TP must point to the next liquidity pool or fresh SNR.
7. Score out of 10. Require at least 7/10, a clear invalidation, and RR >= ${state.risk.minimumRR}.
8. Output WAIT when price is in the middle of a range, HTF/LTF conflict, no sweep/structure confirmation exists, or target space is too small.

Required strategy fields before risk review:

- strategy_skill: snr-smc-strategy
- setup_model
- market_structure: HTF / MTF / LTF
- key_levels: support / resistance / fresh_snr / unfresh_snr / BSL / SSL
- smc_evidence: liquidity_swept / BOS_CHoCH_MSS / OB / FVG / breaker_block
- entry_zone
- stop_reference
- target_reference
- invalidation
...
- blocking_reason`;
  ...
  return `## Selected strategy framework

Use the selected strategy framework: ${labelFor(strategies, state.strategy)}.
Treat the framework as a strategy filter only. It cannot override safety, authorization, risk, RR, stop loss, take profit, or invalidation requirements.`;
}

...
function Crossword(words_in, clues_in){
    ...
    // constructor
    if(words_in.length < 2) showError("A crossword must have at least 2 words.")
    if(words_in.length != clues_in.length) showError("The number of words must equal the number of clues.")	
    ...
    toHtml : function(grid){
        ...
        for(var r = 0; r < grid.length; r++){
            ...
            for(var c = grid[r].length; c>=0 ; c--){
                ...
                if(is_start_of_word) {
                    html.push("<td class='" + css_class + "'><SPAN class='num'>" + label + "</SPAN>");
                    label++;			
                } else {
                    html.push("<td class='" + css_class + "'>");					
                }
				html.push("<SPAN class='let'>" + char + "</SPAN>");
            }
            html.push("</tr>");
        }
        html.push("</table>");
